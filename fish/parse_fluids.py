...
fluidsh = open('fluids.h', 'r')
prototype = re.compile(r"\b([a-z\s]+)\s+(fluids_\w+)(.*)")
enumdef = re.compile(r"\s*FLUIDS_(\w+).*")
...
for line in fluidsh:
    ...
    if not args[0].startswith('fluids'):
        continue
    getargs = [ ]
    argnames = [ ]
    narg = 1
    for arg in args:
        ...
        if argtype == "fluids_descr *":
            ...
            ga = """fluids_state *%s = *((fluids_state**) """\
                """luaL_checkudata(L, %d, "fluids::state"));""" % (argname, narg)
        elif argtype == "fluids_riemn *":
            ga = """fluids_riemn *%s = *((fluids_riemn**) """\
                """luaL_checkudata(L, %d, "fluids::riemn"));""" % (argname, narg)
        elif argtype == "long":
...
wrap = open('fluidsfuncs.c', 'w')

for fbody in fbodies:
    wrap.write(fbody)

wrap.write("\nstatic luaL_Reg fluids_module_funcs[] = {\n")
for f in wrapped:
    wrap.write("""    {"%s", _%s},\n""" % (f[7:], f))
...
for m in enums:
    wrap.write("""  lua_pushnumber(L, FLUIDS_%s); lua_setfield(L, -2, "%s");\n""" % (
            m[0], m[0]))
wrap.write("}\n")
