...
#ifndef __DecomposedCartesianDomain_HEADER__
#define __DecomposedCartesianDomain_HEADER__

#include <mpi.h>
#include "hydro.hpp"

class DecomposedCartesianDomain : public PhysicalDomain
{
protected:
  void create_cart_1d();
  void create_cart_2d();
  ...
  std::string BaseName;
  std::vector<int> neighbors, send_tags, recv_tags;
  MPI_Comm mpi_cart;
...
public:
  ...
  MPI_Comm GetCartComm() const { return mpi_cart; }
  int SubgridAtPosition(const double *r) const;
  int IndexAtPosition(const double *r, int d) const; // d: 0,1,2 for x,y,z
} ;

#endif // __DecomposedCartesianDomain_HEADER__

#else
#include "simple-cart.hpp"
typedef SimpleCartesianDomain DecomposedCartesianDomain;
