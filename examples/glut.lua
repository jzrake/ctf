...
function load_texture(fname)
   ...
   local ppm = buffer.new_buffer(width * height * 3)
   for y = 1, height do
...
ppm, width, height = load_texture('lua-glut/lua.ppm')
...
local mainmenu = glutCreateMenu(function(v) print('mainmenu callback ' .. v) end)
